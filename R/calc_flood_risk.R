...
#' 
#' @param data sf object with polygons of intrest
#' @param dist distance (in meters) of buffer
#' @param save logical, whether to save output
#' @param output_dir character, directory path for saving files
...
calc_flood_risk <-
  function(data,
    ...
    # Input validation
    stopifnot(
      "data must be an sf object" = inherits(data, "sf"),
    ...
    if (st_crs(data) != st_crs(4269)) {
      data <- st_transform(data, crs = 4269)
    }
